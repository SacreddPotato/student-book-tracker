...
- Branch `codex/protected-delete-book-export` adds a minimal password field to the shared student/book deletion dialog. Delete confirmation remains disabled until a password is entered, accepts only the exact case-sensitive password `az2026`, shows a localized inline error for a wrong value, and resets the password/error whenever the dialog is reopened or targets another entity. The password is intentionally client-side and adds no authentication or backend architecture.
...
Book audit option ordering correction (2026-07-17 Cairo):

- Root cause: export choices were sorted by the complete localized `{subject} — {grade}` label, so lexicographic grade text could interleave Preparatory entries among Primary entries and make creation-order fixtures appear random.
...
Excel export header logo checkpoint (2026-07-27 Cairo):

- The supplied `apps/desktop-react/assets/logo.jpeg` is bundled by Vite as an inline data URL and embedded once in both React Excel exports. The student workbook uses its existing dynamic right-side logo span; the book-inventory workbook uses `E1:E3`.
- A shared ExcelJS helper increases header rows 1-3 to 24 points, preserves the source `1080:1063` aspect ratio, constrains the drawing by both available width and height, centers it inside the reserved region, and anchors it without changing either synchronous workbook-builder API.
