...
export function LogsScreen() {
  ...
  const logsQuery = useQuery({ queryKey: logsKey, queryFn: () => backend.listLogs() });
  const [search, setSearch] = useState("");
  const [type, setType] = useState("all");
  const [reversing, setReversing] = useState<LogEntry | null>(null);
  const reverseMutation = useMutation({
    mutationFn: () => backend.reverseTransaction(reversing!.id),
  ...
  return (
    <section className="logs-screen">
      ...
      <div className="log-filters"><div className="log-search"><Search size={17} aria-hidden="true" /><Field label={t("common.search")} value={search} placeholder={t("logs.searchPlaceholder")} onChange={(event) => setSearch(event.target.value)} /></div>
      ...
      {logsQuery.isError ? <Alert>{t("errors.logsLoad")}</Alert> : logsQuery.isPending ? <LoadingState label={t("common.loading")} /> : filtered.length ? <div className="log-list">{filtered.map((log) => <LogCard key={log.id} log={log} onReverse={setReversing} />)}</div> : <EmptyState title={(logsQuery.data?.length ?? 0) ? t("logs.noResults") : t("logs.empty")} />}
