...
export function AcademicYearSetupDialog() {
  const { currentYear, loading, setupReady, initialize } = useAcademicYear();
  const { t } = useI18n();
  ...
  return <Dialog open={!loading && setupReady && !currentYear} onOpenChange={() => {}} title={t("academicYears.setupTitle")} description={t("academicYears.setupDescription")} footer={<Button type="submit" form="academic-year-setup" intent="primary" busy={saving}>{t("academicYears.start")}</Button>}>
