...
import { nextAcademicYear } from "@app/shared";
...
import { AdvanceAcademicYearDialog } from "./AdvanceAcademicYearDialog";
...
export function SettingsScreen() {
  ...
  const targetYear = currentYear ? nextAcademicYear(currentYear) : "";
  ...
  return (
    <section className="settings-screen">
      ...
      <div className="settings-grid">
        {currentYear ? <article className="settings-card"><div className="settings-card-content"><h3>{t("academicYears.settingsTitle")}</h3><p>{t("academicYears.current", { year: currentYear })}</p><Button intent="primary" onClick={() => setAdvanceOpen(true)}>{t("academicYears.advanceTo", { year: targetYear })}</Button></div></article> : null}
      ...
      {currentYear ? <AdvanceAcademicYearDialog targetYear={targetYear} open={advanceOpen} onOpenChange={setAdvanceOpen} /> : null}
    </section>
  );
}
