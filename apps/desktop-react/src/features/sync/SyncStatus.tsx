...
export function SyncStatus() {
  ...
  return (
    <>
      <button type="button" className="sync-status-button" data-phase={status.phase} onClick={() => setOpen(true)}><Icon size={16} aria-hidden="true" /><span>{label}</span>{status.unacknowledgedRejectedCount ? <Badge tone="danger">{status.unacknowledgedRejectedCount}</Badge> : null}</button>
